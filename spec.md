...
- 収益分配
  - 各オーナーに持分（`share`、小数2桁の百分率）を割り当て、合計を 100.00 に固定する。
...
- PRIMARY 移譲
  ...
  - 移譲確定時、譲渡元は MANAGER に降格（または共同オーナーから離脱）する選択ができる。
  - 移譲先プレイヤーが `shop.maxShopsPerPlayer` の上限に達している場合は移譲を拒否する。
...
- サブコマンド:
  ...
  - `/vshop egg <player> <lines|"inf"|"admin">`: ショップスポーンエッグ配布
    - `lines`: 出品枠の行数（1〜の整数、9スロット×行数の枠を持つショップになる）
    - `"inf"`: 行数無制限ショップ用エッグ（ページング前提）
    - `"admin"`: 管理者ショップ用エッグ
...
- Dialogフロー:
  ...
  - 共同オーナー管理フロー（PRIMARY 専用、§3.6）
    ...
    - 削除: ConfirmDialog
    - 持分合計が 100.00 にならない場合は確定時にエラー再入力を促す
  - PRIMARY 移譲フロー（PRIMARY 専用）
    - プレイヤー選択チェストUI（後述）→ 譲渡元の扱い選択（MANAGER に降格 / 共同オーナーから離脱）→ ConfirmDialog
- 数量・価格入力UX
...
- チェストUI利用フロー:
  ...
  - 出品枠の登録・編集（編集モード、オーナー / MANAGER 専用、`/vshop edit` 起点）
    ...
    - 空スロットにプレイヤーが自身のインベントリからアイテムを置く（ドロップ／シフトクリック等）と、対象アイテムとして取り込む → 種別選択 → 価格・数量入力 Dialog → 確定で当該 `slot_index` に新規枠を登録
      - 取り込んだアイテムは**プレイヤーのインベントリから消費しない**（参照用にコピーを保持し、元のスタックはそのまま手元に残す）
      - チェストUIのスロット上には登録予定の見本としてそのアイテムを表示する
      - Dialog をキャンセル or 閉じた場合は見本表示を破棄し、空スロットに戻す
      - 取引禁止アイテム（§3.5）や数量0のスタックを置こうとした場合は、Dialog 起動前にエラー案内し見本表示しない
    - 枠削除は編集 Dialog 内の「削除」ボタン経由とし、誤操作防止のため ConfirmDialog を挟む
    - `slot_index` は (page * 27 + slot) のフラットな整数として保存し、ページング後も一意性を保つ
  ...
  - プレイヤー選択チェストUI
    ...
    - キャッシュ未登録プレイヤーの選択: 検索 InputDialog で入力された名前がキャッシュにヒットしない場合、確認 ConfirmDialog 経由で「未参加プレイヤーとして扱う」選択を許可する
      - 未参加プレイヤー指定時は `OfflinePlayer#getUniqueId` で UUID 解決を試み、成功すれば選択候補として確定。失敗時はエラー案内
    - スキン取得は非同期で実行し、未取得時は `MHF_Question` 等のデフォルト頭で代替表示
...
- 主要エンティティ（概略）
  ...
  - `shop_slots`: 出品枠（ショップID・slot_index・種別SELL/BUY/BOTH・アイテムBLOB・単価・数量上限・取引上限・上限スコープ・リセット周期）
    - `slot_index` はチェストUIの (page * 27 + slot) を保持し、編集モードと閲覧モードで同じ座標表現を共有する
    - `(shop_id, slot_index)` を一意制約とする
  - `shop_inventory`: プレイヤーショップの在庫（ショップID・スロット位置・アイテムBLOB・数量）
    - スロット位置を保持し、チェストUIでの配置を維持する
