...
public final class SqliteCoOwnerRepository implements CoOwnerRepository {

    private final DataSource dataSource;

    public SqliteCoOwnerRepository(DataSource dataSource) {
    ...
    public void upsert(CoOwner owner) throws SQLException {
        try (Connection c = dataSource.getConnection()) {
            ...
            try (PreparedStatement ps = c.prepareStatement(
                    "SELECT 1 FROM shop_co_owners WHERE shop_id = ? AND player_uuid = ?")) {
            ...
            if (exists) {
                try (PreparedStatement ps = c.prepareStatement(
                        "UPDATE shop_co_owners SET role=?, share_pct=? WHERE shop_id=? AND player_uuid=?")) {
                    ps.setString(1, owner.role().name());
                    ps.setBigDecimal(2, owner.share());
                    ps.setString(3, owner.shopId().toString());
                    ps.setString(4, owner.playerUuid().toString());
                    ps.executeUpdate();
                }
            } else {
                try (PreparedStatement ps = c.prepareStatement(
